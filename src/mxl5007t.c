...
static int mxl5007t_probe(struct i2c_client *client)
{
	struct mxl5007t_config *config = client->dev.platform_data;
	...
	if (mxl5007t_attach(config->frontend, client->adapter, client->addr, config) == NULL) {
		dev_err(&client->dev, "%s: mxl5007t_attach failed\n", __func__);
		return -ENODEV;
	}

	i2c_set_clientdata(client, fe->tuner_priv);

	dev_info(&client->dev, "%s: mxl5007t tuner probed\n", __func__);

	return 0;
}

static void mxl5007t_remove(struct i2c_client *client)
{
	struct mxl5007t_state *mxl5007t = i2c_get_clientdata(client);

	if (mxl5007t) {
		mxl5007t_release(mxl5007t->config->frontend);
	}
}

static const struct i2c_device_id mxl5007t_id_table[] = {
	{ "mxl135rf", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, mxl5007t_id_table);

static struct i2c_driver mxl5007t_driver = {
	.driver = {
		.name = "mxl135rf",
	},
	.probe    = mxl5007t_probe,
	.remove   = mxl5007t_remove,
	.id_table = mxl5007t_id_table,
};

module_i2c_driver(mxl5007t_driver);
