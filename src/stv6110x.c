...
static int stv6110x_set_frequency(struct dvb_frontend *fe, u32 frequency)
{
	...
	dev_dbg(&d->udev->dev, "%s: setting frequency to %u kHz\n", __func__, frequency);
...
static int stv6110x_set_bandwidth(struct dvb_frontend *fe, u32 bandwidth)
{
	...
	struct dvb_usb_device *d = fe_to_d(fe);
	dev_dbg(&d->udev->dev, "%s: setting bandwidth to %u kHz\n", __func__, bandwidth);
...
static void stv6110x_init_regs(struct stv6110x_state *stv6110x)
{
...
static int stv6110x_set_params(struct dvb_frontend *fe)
{
	struct stv6110x_state *stv6110x = fe->tuner_priv;
	struct dtv_frontend_properties *p = &fe->dtv_property_cache;
	int ret = 0;

	stv6110x_init_regs(stv6110x);
	stv6110x_setup_divider(stv6110x);

	ret = stv6110x_init(fe);
	if (ret < 0) {
		return ret;
	}

	ret = stv6110x_set_frequency(fe, p->frequency);
	if (ret < 0) {
		return ret;
	}

	ret = stv6110x_set_bandwidth(fe, p->bandwidth_hz);
	if (ret < 0) {
		return ret;
	}
...
static const struct dvb_tuner_ops stv6110x_ops = {
	...
	.set_params		= stv6110x_set_params,
	.get_status		= stv6110x_get_status,
	.get_frequency	= stv6110x_get_frequency,
	.get_bandwidth	= stv6110x_get_bandwidth,
...
MODULE_DESCRIPTION("STV6110A Silicon tuner");
MODULE_LICENSE("GPL");
